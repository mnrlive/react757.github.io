  ...
  render() {
    const routeItems = this.state.itemsAll.map((item, index) => (
      <Route exact path={`/${item.alpha3Code}`} key={index}
        render={() => <main className="main main--country"><Country itemsAll={this.state.itemsAll} flag={item.flag} name={item.name} nativeName={item.nativeName} population={item.population}
